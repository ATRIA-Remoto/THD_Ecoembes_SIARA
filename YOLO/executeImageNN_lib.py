...
def pintarRectangulo(frame, bbox, color, tag=None, grosor=2, textType=cv2.FONT_HERSHEY_SIMPLEX, scale=0.5):
    (x, y) = (int(bbox[0]), int(bbox[1]))
    (w, h) = (int(bbox[2]), int(bbox[3]))
    cv2.rectangle(frame, (x, y), (x + w, y + h), color, grosor)
    if tag is not None:
        cv2.putText(frame, tag, (x, y - 5), textType, scale, color, grosor)
    return frame
...
class YOLONet:
    ...
    def showResultsNMS(self, img, boxes, classes, scores, max_boxes_to_draw=200, min_score_thresh=0.7):

        if self.labels is None:
            self.colors = []
            random.seed(time.time())
            for cl in classes:
                self.colors.append((random.random()*255, random.random()*255, random.random()*255))
        else:
        ...
        cl_detected = []
        [cl_detected.append(i) for i in classes if i not in cl_detected]

        boxes_show = []
        cl_show = []
        for cl in cl_detected:
            boxes_cl = [box for i, box in enumerate(boxes) if classes[i]==cl]

            nmsBoxes = np.array([np.array([xtl*np.shape(image_np_with_detections)[1], ytl*np.shape(image_np_with_detections)[0], xbr*np.shape(image_np_with_detections)[1], ybr*np.shape(image_np_with_detections)[0]]) for (ytl, xtl, ybr, xbr) in boxes_cl])

            nmsBoxes = non_max_suppression(nmsBoxes)

            nmsBoxes = [np.array([xtl, ytl, xbr-xtl, ybr-ytl]) for (xtl, ytl, xbr, ybr) in nmsBoxes]
            for box in nmsBoxes:
                cl_show.append(cl)
                boxes_show.append(box)
        
        for i, box in enumerate(boxes_show):
            image_np_with_detections = pintarRectangulo(image_np_with_detections, box, self.colors[cl_show[i]], tag=self.labels[cl_show[i]], grosor=4, scale=1)
