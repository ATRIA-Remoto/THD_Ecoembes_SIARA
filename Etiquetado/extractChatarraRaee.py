...
ONLY_LABELED = True
SAVE_IMAGES = "D:\Proyectos\THD_Ecoembes\Imagenes/chatarraee"
...
DIR='A:/00_ATRIA/11_Proyectos Financiados/02_Proyectos Presentados/01_Aceptados/2019_03_THD_Ecoembes_SIARA/04_PROYECTO/05_Documentos de trabajo/03_Imágenes\Records'
files = [
    "20201027_1.tfrecord"
]
...
for (i, tfrecord) in enumerate(files):
    ...
    if extension == 'tfrecord':
        ...
        for string_record in record_iterator:
            k = k + 1
        record_iterator = tf.compat.v1.python_io.tf_record_iterator(tfrecord)
        widgets = ["Extracting images (file " + str(i+1) + "/" + str(filesTotal) + "): ", progressbar.Percentage(), " ", progressbar.Bar(), " ", progressbar.ETA()]
        ...
        for (j, string_record) in enumerate(record_iterator):
            ...
            if ONLY_LABELED:
                ...
                if len(label) > 0:
                    for l in example.features.feature["image/object/class/text"].bytes_list.value:
                        if l.decode('utf-8') == 'chatarra' or l.decode('utf-8') == 'raee':
                            save_image = True
